...
class Train_Setup():
    ...
    def one_epoch(self, step, train_T_out, test_T_out, dt=0):
        ...
        for xx, yy in self.train_loader:
            ...
            loss = 0
            xx[0] = xx[0].to(self.device, non_blocking=True)
            xx[1] = xx[1].to(self.device, non_blocking=True)
        ...
        with torch.no_grad():
            for xx, yy in self.test_loader:
                xx[0], xx[1], yy = xx[0].to(self.device, non_blocking=True), xx[1].to(self.device, non_blocking=True), yy.to(self.device, non_blocking=True)
                batch_size = xx[0].shape[0]
...
# %%
class Eval_Setup():
    def __init__(self, model, test_loader, test_out, normalizer=None, ode_solver='custom', 
    ...
    def inference(self, step, T_out, eval_metric='MSE', dt=0):
        ...
        with torch.no_grad():
         ...
         # Fix: Update the list elements properly
                xx = [xx[0].to(self.device, non_blocking=True), 
                  xx[1].to(self.device, non_blocking=True)]
                yy = yy.to(self.device, non_blocking=True)
                batch_size = xx[0].shape[0]
