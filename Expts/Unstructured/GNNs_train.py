...
# -*- coding: utf-8 -*-
"""
GNO Testing Pipeline with Simvue integration.
"""
#Imports
...
with Run(mode='offline') as run:
    ...
    run.init(
        ...
        tags=['GNO', configuration['Model']['arch'], 'Test', configuration['Physics']['pde']], 
        metadata=run_config
    )
    ...
    pred_set = pred_set.reshape(pred_set.shape[0], pred_set.shape[1], pred_set.shape[2], 33, 33).cpu()

    test_out = data_test[...,1:].permute(0, 1, 3, 2)
    test_out = test_out.reshape(test_out.shape[0], test_out.shape[1], test_out.shape[2], 33, 33).cpu()

    mse_error = np.mean(pred_set.numpy()-test_out.numpy())**2
    print(f"MSE: {mse_error}")
    # Log final MSE error to Simvue metadata
    run.update_metadata({'MSE Test Error (approx)': float(mse_error)})
