...
if pde == 'incompressible':
    if study == 'data-efficiency':
        ar = ['wide-timer', 'young-axe', 'active-offense', 'giant-skill']
        euler = ['happy-walk', 'humble-stack', 'direct-dodecagon', 'clever-ball']
        ops_split = ['reduced-roundel', 'simple-taste', 'cold-ordnance', 'wan-snipe']
        x_axis = [100, 200, 400, 500]
        xlabel = 'Training Samples'
        train_times = np.array([6200, 12500, 24500, 31000])/3600

    if study == 'model-efficiency':
        ar = ['ebony-villa', 'arctic-vector', 'wide-timer']
        euler = ['feasible-light', 'amber-burrito', 'happy-walk']
        ops_split = ['proper-coupe', 'exhaustive-party', 'reduced-roundel']
        x_axis = [2, 4, 6]
        xlabel = 'Number of Layers'
        train_times = np.array([2600, 4500, 6200])/3600

    if study == 'rollout-length':
        ar = ['tart-tunnel', 'visible-vector', 'tense-anthology', 'dry-image']
        euler = ['average-boom', 'pure-curve', 'glossy-cyclist', 'nutty-raclette']
        ops_split = ['boxy-landaulet', 'pale-bosun', 'trusting-shape', 'tender-analogy']
        x_axis = [1, 5, 15, 25]
...
if pde == 'compressible':
    if study == 'data-efficiency':
        ar = ['obnoxious-yard', 'bisque-stock', 'blue-wharf', 'sophisticated-refinery']
        euler = ['beige-bocaccio', 'unary-polygon', 'icy-silo', 'beige-value']
        ops_split = ['terminal-rehab', 'medium-duffel', 'dark-promo', 'resultant-cement']
        x_axis = [100, 200, 400, 500]
        xlabel = 'Training Samples'
...
for index, type in enumerate(types):
    ...
    for run in type:
        ...
        #Defining the ops split values
        from operator_splitting_ablations import *
        dist(data_dist)
